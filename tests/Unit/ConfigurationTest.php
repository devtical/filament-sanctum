...
class ConfigurationTest extends TestbenchTestCase
{
    ...
    public function test_abilities_configuration()
    {
        ...
        $this->assertArrayHasKey('allow_expiration', $abilities);
        $this->assertArrayHasKey('default_expiration_days', $abilities);
        $this->assertArrayHasKey('expiration_presets', $abilities);

        $this->assertIsInt($abilities['columns']);
        $this->assertGreaterThan(0, $abilities['columns']);
        $this->assertIsArray($abilities['list']);
        $this->assertIsBool($abilities['allow_expiration']);
        $this->assertIsArray($abilities['expiration_presets']);
    }

    public function test_authorization_configuration()
    {
        $authorization = $this->app['config']->get('filament-sanctum.authorization');

        $this->assertIsArray($authorization);
        $this->assertArrayHasKey('enabled', $authorization);
        $this->assertArrayHasKey('gate', $authorization);
        $this->assertIsBool($authorization['enabled']);
    }
    ...
    public function test_default_config_values_when_not_overridden()
    {
        ...
        $this->assertTrue($config['abilities']['allow_expiration']);
        $this->assertEquals(30, $config['abilities']['default_expiration_days']);
        $this->assertEquals([7, 30, 60, 90], $config['abilities']['expiration_presets']);
        $this->assertFalse($config['authorization']['enabled']);
    }
}
